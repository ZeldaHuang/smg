        ...
        let workers = match self.mode {
            WorkerSelectionMode::Regular => {
                match self.select_single_worker(
                    ...
                    None => return Err(self.regular_failure(model_id, lane)),
                }
            }
        ...
        let workers = match self.mode {
            WorkerSelectionMode::Regular => {
                match self.select_single_worker(
                    ...
                    Some(w) => WorkerSelection::Single { worker: w },
                    None => return Err(self.regular_failure(model_id, LaneFilter::Wire(ctx.wire))),
                }
            }
...
impl WorkerSelectionStage {
    ...
    fn regular_failure(&self, model_id: &str, lane: LaneFilter) -> Response {
        let unsupported = match lane {
            LaneFilter::Any => {
                return self.selection_failure(model_id, &[WorkerType::Regular], None);
            }
            LaneFilter::Wire(wire) => {
                return self.selection_failure(model_id, &[WorkerType::Regular], Some(wire));
            }
...
mod tests {
    ...
    fn requests_the_two_tier_lane_cannot_serve_stay_off_smg_workers() {
        ...
        for _ in 0..4 {
            let picked = stage
                .select_single_worker(model_id, None, None, None, None, None, engine_only)
    ...
    async fn a_request_the_smg_lane_cannot_serve_is_501_on_an_smg_only_pool() {
        ...
        assert!(stage
            .select_single_worker(model_id, None, None, None, None, None, lane)
            .is_none());
        let response = stage.regular_failure(model_id, lane);
        ...
        // A model nobody serves stays a 404.
        assert_eq!(
            stage.regular_failure("no-such-model", lane).status(),
            StatusCode::NOT_FOUND
        );
    }
