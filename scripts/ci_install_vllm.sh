...
# installs). 0.29.0 keeps the properties the lane depends on: torchcodec is
...
# route; verified in 0.29.0's dependency constraints).
# --torch-backend=auto matches the torch CUDA variant to the pod's driver.
echo "Installing vLLM..."
$RETRY 3 10 uv pip install "vllm==0.29.0" --torch-backend=auto
